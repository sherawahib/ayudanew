...
export default function Header() {
  ...
  return (
    ...
    >
      <TopBar />
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-5 xl:px-6">
        <div className="flex items-center justify-between gap-3 py-2 sm:gap-4 sm:py-3">
          <Link href="/" className="shrink-0">
            <Image
              ...
              width={540}
              height={180}
              className="h-[72px] w-auto sm:h-[80px] xl:h-[88px] 2xl:h-[104px]"
              priority
            />
          </Link>

          <nav className="hidden min-w-0 flex-1 xl:block" aria-label="Main navigation">
            <ul className="flex items-center justify-end gap-x-0">
              {NAV_ITEMS.map((item) => (
                <li key={item.label} className="group relative shrink-0">
                  <Link
                    href={item.href}
                    className={`flex items-center gap-0.5 whitespace-nowrap px-1.5 py-2.5 text-[10px] font-medium uppercase tracking-wide transition-colors hover:text-ayuda-blue 2xl:gap-1 2xl:px-2 2xl:py-3 2xl:text-[11px] ${
                  ...
                  >
                    ...
                    {item.children && (
                      <svg
                        className="mt-0.5 h-2.5 w-2.5 opacity-60 2xl:h-3 2xl:w-3"
                        viewBox="0 0 10 6"
                        fill="currentColor"
                        aria-hidden
                      >
              ...
              <li className="shrink-0">
                <Link
                  href={SITE.phoneHref}
                  className="flex items-center gap-1 whitespace-nowrap px-1.5 py-2.5 text-[10px] font-medium text-black transition-colors hover:text-ayuda-blue 2xl:gap-1.5 2xl:px-2 2xl:py-3 2xl:text-[11px]"
                  aria-label={`Call ${SITE.phone}`}
                >
                  <svg className="h-3 w-3 shrink-0 2xl:h-3.5 2xl:w-3.5" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
