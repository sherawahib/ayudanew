...
export default function SocialAuthButtons() {
  ...
  const facebookEnabled = Boolean(process.env.NEXT_PUBLIC_FACEBOOK_ENABLED);
  ...
  async function handleSocial(provider: "google" | "facebook") {
    setLoading(provider);
    await signIn(provider, { callbackUrl: "/donors/dashboard" });
  }
      ...
      <div className="grid gap-3">
        ...
        {facebookEnabled ? (
          <button
            ...
            disabled={loading !== null}
            className="flex w-full items-center justify-center gap-3 rounded-sm border border-[#1877f2] bg-[#1877f2] px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-[#166fe0] disabled:opacity-60"
          >
